    ...
    public static void processLogFile(String logFilePath) {
                        ...
                        try {
                            ...
                            if (status.equals("Start")) {
                                ...
                                if (stack != null && !stack.isEmpty()) {
                                    long duration = sessionDurations.getOrDefault(username, 0L);
                                    duration += Duration.between(stack.pop(), time).getSeconds();
        ...
        if (earliestTime != null && latestTime != null) {
            for (String username : sessionStacks.keySet()) {
                ...
                while (!stack.isEmpty()) {
                    long duration = sessionDurations.getOrDefault(username, 0L);
                    duration += Duration.between(stack.pop(), latestTime).getSeconds();
                    sessionDurations.put(username, duration);
                }
            }
        }
        ...
        for (String username : sessionCounts.keySet()) {
            ...
            System.out.printf("%s %d %d%n", username, sessionCount, sessionDuration);
        }
    }
}